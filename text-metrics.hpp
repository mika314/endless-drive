...
public:
  ...
  void appendText(Atlas &, FontHandle _fontHandle, const char *_string);

  /// Append a wide char string to the metrics helper.
  void appendText(Atlas &, FontHandle _fontHandle, const wchar_t *_string);
...
public:
  ...
  void getSubText(const bx::StringView &_str,
                  uint32_t _firstLine,
                  uint32_t _lastLine,
                  const char *&_begin,
                  const char *&_end);

...
RectanglePacker::RectanglePacker(uint32_t _width, uint32_t _height)
  : m_width(_width), m_height(_height), m_usedSpace(0)
{
    ...
    m_maxRegionCount(_maxRegionsCount)
{
  ...
  BX_ASSERT(_maxRegionsCount >= 64 && _maxRegionsCount <= 32000,
            "Invalid _maxRegionsCount %d.",
            _maxRegionsCount);
...
void Atlas::updateRegion(const AtlasRegion &_region, const uint8_t *_bitmapBuffer)
{
  ...
  if (0 < size)
  {
    ...
    if (_region.getType() == AtlasRegion::TYPE_BGRA8)
    {
      ...
      uint8_t *outLineBuffer = m_textureBuffer +
                               _region.getFaceIndex() * (m_textureSize * m_textureSize * 4) +
    ...
    else
    {
      ...
      const uint8_t *inLineBuffer = _bitmapBuffer;
      uint8_t *outLineBuffer =
        (m_textureBuffer + _region.getFaceIndex() * (m_textureSize * m_textureSize * 4) +
         (((_region.y * m_textureSize) + _region.x) * 4));
...
void Atlas::packFaceLayerUV(uint32_t _idx,
...
void Atlas::packUV(uint16_t _regionHandle,
                   uint8_t *_vertexBuffer,
                   uint32_t _offset,
                   uint32_t _stride) const
{
...
void Atlas::packUV(const AtlasRegion &_region,
                   uint8_t *_vertexBuffer,
                   uint32_t _offset,
                   uint32_t _stride) const
{

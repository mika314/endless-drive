...
public:
  ...
  float getOutlineWidth() { return m_outlineWidth; }

  uint32_t getDropShadowColor() { return m_dropShadowColor; }

  float getDropShadowOffsetU() { return m_dropShadowOffset[0]; }

  float getDropShadowOffsetV() { return m_dropShadowOffset[1]; }

  float getDropShadowSoftener() { return m_dropShadowSoftener; }
  ...
  void setOutlineColor(uint32_t _rgba = 0x000000FF) { m_outlineColor = toABGR(_rgba); }

  void setOutlineWidth(float _outlineWidth = 3.0f) { m_outlineWidth = _outlineWidth; }

  void setDropShadowColor(uint32_t _rgba = 0x000000FF) { m_dropShadowColor = toABGR(_rgba); }
  ...
  */
  ...
  void setDropShadowSoftener(float smoother) { m_dropShadowSoftener = smoother; }
  ...
  /// position and color.
  void appendText(Atlas &, const Font &, int size, const char *_string, const char *_end = NULL);

  /// Append a wide char unicode string to the buffer using current pen
  /// position and color.
  void appendText(Atlas &, const Font &, int size, const wchar_t *_string, const wchar_t *_end = NULL);
  ...
  uint32_t getVertexSize() const { return sizeof(TextVertex); }

  /// get a pointer to the index buffer to submit it to the graphic
  const uint16_t *getIndexBuffer() const { return m_indexBuffer; }
  ...
  uint32_t getTextColor() const { return toABGR(m_textColor); }

  TextRectangle getRectangle() const { return m_rectangle; }

private:
  void appendGlyph(Atlas &, const Font &, int size, CodePoint _codePoint, bool shadow);
  void verticalCenterLastLine(float _txtDecalY, float _top, float _bottom);

  static uint32_t toABGR(uint32_t _rgba)
  {
    return (((_rgba >> 0) & 0xff) << 24) | (((_rgba >> 8) & 0xff) << 16) |
           (((_rgba >> 16) & 0xff) << 8) | (((_rgba >> 24) & 0xff) << 0);
  }
...
void TextBuffer::appendText(Atlas &atlas,
                            const Font &font,
                            int size,
                            const char *_string,
                            const char *_end)
{
  ...
  for (; *_string && _string < _end; ++_string)
  {
    ...
    {
      appendGlyph(atlas, font, size, codepoint, false);
    }
  }
                            ...
                            const wchar_t *_end)
{
  ...
  for (const wchar_t *_current = _string; _current < _end; ++_current)
  {
    ...
    appendGlyph(atlas, font, size, _codePoint, false);
  }
}
...
void TextBuffer::appendGlyph(Atlas &atlas, const Font &font, int size, CodePoint _codePoint, bool shadow)
{
  if (_codePoint == L'\t')
  {
    for (uint32_t ii = 0; ii < 4; ++ii)
    {
      appendGlyph(atlas, font, size, L' ', shadow);
    }
    return;
  }
  ...
  const auto glyph = font.getGlyphInfo(atlas, _handle, _codePoint);
  ...
  const auto blackGlyphRegionIndex = atlas.getBlackGlyphRegionIndex();
  ...
  if (shadow)
  {
    if (atlasRegion.getType() != AtlasRegion::TYPE_BGRA8)
    {
      ...
      uint32_t adjustedDropShadowColor =
        ((((m_dropShadowColor & 0xff000000) >> 8) * (m_textColor >> 24)) & 0xff000000) |
        (m_dropShadowColor & 0x00ffffff);
  ...
  if (m_styleFlags & STYLE_BACKGROUND && m_backgroundColor & 0xff000000)
  {
    ...
    atlas.packUV(blackGlyphRegionIndex,
                 (uint8_t *)m_vertexBuffer,
...
TextBufferManager::~TextBufferManager()
{
  BX_ASSERT(m_textBufferHandles.getNumHandles() == 0,
...
void TextBufferManager::submitTextBuffer(TextBufferHandle _handle, bgfx::ViewId _id, int32_t _depth)
{
  ...
  case BufferType::Static: {
    ...
    if (bgfx::kInvalidHandle == bc.vertexBufferHandleIdx)
    {
      ...
      vbh = bgfx::createVertexBuffer(bgfx::copy(bc.textBuffer->getVertexBuffer(), vertexSize),
                                     m_vertexLayout);
  ...
  case BufferType::Dynamic: {
    ...
    if (bgfx::kInvalidHandle == bc.vertexBufferHandleIdx)
    {
      ...
      vbh = bgfx::createDynamicVertexBuffer(bgfx::copy(bc.textBuffer->getVertexBuffer(), vertexSize),
                                            m_vertexLayout);
...
void TextBufferManager::appendText(Atlas &atlas,
                                   ...
                                   const char *_end)
{
  BX_ASSERT(isValid(_handle), "Invalid handle used");
  BufferCache &bc = m_textBuffers[_handle.idx];
  bc.textBuffer->appendText(atlas, font, size, _string, _end);
}

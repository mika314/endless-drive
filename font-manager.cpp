...
public:
  ...
  /// @remark The ownership of the buffer is external, and you must ensure it stays valid up to this
  /// object lifetime
  /// @return true if the initialization succeed
  bool init(const uint8_t *_buffer,
            uint32_t _bufferSize,
            int32_t _fontIndex,
            uint32_t _pixelHeight,
            int16_t _widthPadding,
...
bool TrueTypeFont::init(const uint8_t *_buffer,
                        uint32_t _bufferSize,
                        int32_t _fontIndex,
                        uint32_t _pixelHeight,
                        int16_t _widthPadding,
                        int16_t _heightPadding)
{
  BX_WARN((_bufferSize > 256 && _bufferSize < 100000000),
          "(FontIndex %d) TrueType buffer size is suspicious (%d)",
          _fontIndex,
          _bufferSize);
  BX_WARN((_pixelHeight > 4 && _pixelHeight < 128),
...
using GlyphHashMap = tinystl::unordered_map<CodePoint, GlyphInfo>;

// cache font data
struct FontManager::CachedFont
{
  CachedFont() : trueTypeFont(NULL) { masterFontHandle.idx = bx::kInvalidHandle; }
...
FontManager::~FontManager()
{
  BX_ASSERT(m_fontHandles.getNumHandles() == 0,
...
FontHandle FontManager::createFontByPixelSize(TrueTypeHandle _ttfHandle,
                                              uint32_t _typefaceIndex,
                                              uint32_t _pixelSize,
                                              uint32_t _fontType,
                                              uint16_t _glyphWidthPadding,
                                              uint16_t _glyphHeightPadding)
{
  BX_ASSERT(isValid(_ttfHandle), "Invalid handle used");

  TrueTypeFont *ttf = new TrueTypeFont();
  if (!ttf->init(m_cachedFiles[_ttfHandle.idx].buffer,
                 m_cachedFiles[_ttfHandle.idx].bufferSize,
                 _typefaceIndex,
                 _pixelSize,
                 _glyphWidthPadding,
                 _glyphHeightPadding))
...
bool FontManager::preloadGlyph(Atlas &atlas, FontHandle _handle, CodePoint _codePoint)
{
  ...
  if (NULL != font.trueTypeFont)
  {
    ...
    {
    case FONT_TYPE_ALPHA: font.trueTypeFont->bakeGlyphAlpha(_codePoint, glyphInfo, m_buffer); break;

...
TextMetrics::TextMetrics(FontManager *_fontManager) : m_fontManager(_fontManager)
{
  clearText();
}
...
void TextMetrics::appendText(Atlas &atlas, FontHandle _fontHandle, const char *_string)
{
  ...
  for (; *_string; ++_string)
  {
    ...
    {
      const GlyphInfo *glyph = m_fontManager->getGlyphInfo(atlas, _fontHandle, codepoint);
      if (NULL != glyph)
      {
        if (codepoint == L'\n')
        {
...
void TextLineMetrics::getSubText(const bx::StringView &_str,
                                 uint32_t _firstLine,
                                 uint32_t _lastLine,
                                 const char *&_begin,
                                 const char *&_end)
{
  ...
  while (ptr != _str.getTerm() && (currentLine < _firstLine))
  {
  ...
  while (ptr != _str.getTerm() && (currentLine < _lastLine))
  {
    for (; ptr != _str.getTerm(); ++ptr)
    {
      if (utf8_decode(&state, (uint32_t *)&codepoint, *ptr) == UTF8_ACCEPT)

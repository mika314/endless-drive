...
struct AtlasRegion
{
  ...
  uint32_t
    mask; // encode the region type, the face index and the component index in case of a gray region
...
public:
  ...
  /// @param textureSize an atlas creates a texture cube of 6 faces with size equal to
  /// (textureSize*textureSize * sizeof(RGBA) )
  ...
  uint16_t addRegion(uint16_t _width,
                     uint16_t _height,
  ...
  /// Pack the UV coordinates of the four corners of a region to a vertex buffer using the supplied
  /// vertex format. v0 -- v3 |     |     encoded in that order:  v0,v1,v2,v3 v1 -- v2
  /// @remark the UV are four signed short normalized components.
  /// @remark the x,y,z components encode cube uv coordinates. The w component encode the color channel
  /// if any.
  /// @param handle handle to the region we are interested in
  /// @param vertexBuffer address of the first vertex we want to update. Must be valid up to vertexBuffer
  /// + offset + 3*stride + 4*sizeof(int16_t), which means the buffer must contains at least 4 vertex
  /// includind the first.
  ...
  void packUV(const AtlasRegion &_region,
              uint8_t *_vertexBuffer,
              uint32_t _offset,
              uint32_t _stride) const;

  /// Same as packUV but pack a whole face of the atlas cube, mostly used for debugging and visualizing
  /// atlas
  ...
  uint16_t getTextureSize() const { return m_textureSize; }
